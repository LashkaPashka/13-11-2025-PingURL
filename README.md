# Go Link Checker

## Описание проекта

**Go Link Checker** — это веб-сервис на Go, который позволяет:

- Проверять доступность интернет-ресурсов по ссылкам.
- Отправлять как одну ссылку, так и сразу несколько.
- Формировать PDF-отчёт по статусам ссылок для выбранных задач.
- Сохранять состояние задач между перезапусками сервера, чтобы незавершённые задачи продолжали выполняться.

Все данные сохраняются в локальном JSON-файле (`storage/storage.json`).

## Установка

1. Клонируйте репозиторий:

```bash
git clone git@github.com:LashkaPashka/13-11-2025-PingURL.git
cd 13-11-2025-PingURL.git
```

2. Установите переменные окружения в файле (`config/local.yaml`).
```yaml
env: "local"
storage_path: "./storage/storage.json"
http_server:
  address: "0.0.0.0:8080"
  timeout: 4s
  idle_timeout: 30s
```

3. Установка зависимостей.
```bash
go mod tidy
```

4. Запуска сервера.
```bash
CONFIG="./config/local.yaml" go run cmd/app/main.go
```

## REST API
1. POST `/url-check` — отправка ссылок для проверки

Request:
```json
{
	"links": [
		"wfsfe.gg",
		"google.com"
	]
}
```

Response:
```json
{
	"links": [
		{
			"status_code": 404,
			"status": "failed",
			"url_name": "wfsfe.gg",
			"available": "down"
		},
		{
			"status_code": 200,
			"status": "done",
			"url_name": "google.com",
			"available": "up"
		}
	],
	"status": "done",
	"links_num": "4543e43b-8db1-405e-9b28-2535b5093a67"
}
```

2. POST `/get-info-links` - получение информации о ссылках в pdf файле

Request:
```json
{
	"links_list": [
		"4543e43b-8db1-405e-9b28-2535b5093a67"
	]
}
```

Response:
```pdf
Link Check Report
=======================
https://wfsfe.gg - down
https://google.com - up
```

## Устойчивость к сбоям

### Сохранение прогресса

Все изменения записываются в `storage.json` сразу после проверки ссылки.

### Продолжение незавершённых задач

При старте сервера все ссылки со статусом `"pending"` идут на проверку.

### Атомарная запись

Перед записью создаётся временный файл, который затем заменяет основной (`os.WriteFile` + `os.Rename`).

### Graceful shutdown

Можно корректно остановить сервер, отправив ему сигналы SIGINT или SIGTERM, так как реализуется graceful shutdown — сервер завершает обработку текущих запросов и корректно останавливает все активные задачи

